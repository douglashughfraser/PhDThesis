...
bool Activated_Flood_Control = 1;
bool Return_Water_Supply_Control = 0;
bool Gen_A_Status = 1;
bool Tag_2 = 0;
bool HMI_Return_Feed = 0;
...
proctype hydro_generator()
{
	...
	/* Since the remaining behaviour is in response to operator actions
	/* wait until operator interaction has been taken before proceeding */
	/* OperatorAction == true; */
...
proctype hmi()
{
	...
	/* Wait until PLCs have finished reacting */
	/* ControlDone == true && GeneratorDone == true; */
...
init{
	/* Since we do not have PLC mode data we present it as a non-deterministic option which state the PLC are in */
	if
